...
`.patternitty/signal.jsonl` in the project you're working in, wired into
...
- **Copilot** — the `userPromptSubmitted` hook (`.github/hooks/patternitty-capture.json`)
...
The `patternitty` skill (`skills/patternitty/SKILL.md`) reads
`.patternitty/signal.jsonl` and matches it against your pattern store. Genuine
...
rules to modular per-cluster files (`patternitty/<cluster>.md`, the single
source), then makes each tool config **reference** them rather than inlining:
`CLAUDE.md` gets `@patternitty/<cluster>.md` imports; `AGENTS.md` and the
...
`${PATTERNITTY_HOME:-~/.patternitty}/patterns/index.json` is the machine-readable
